...
export default (props) => {
  ...
  let sortProd = prod.sort(function (a, b) {
          ...
          onClick={() => {
            axios
              ...
              .then(({ data }) => {
                ...
                ).filter((product) => product._id !== data.payload._id);
  ...
  const [showEditModal, hideEditModal] = useModal(({ in: open, onExited }) => (
    <Dialog open={open} onExited={onExited} onClose={hideEditModal}>
      ...
      <DialogContent>
        <Add
          method="edit"
          object={prods.find((prod) => prod._id === itemId)}
              ...
              {prods
                ...
                .map((item, index) => (
                  <tr key={index}>
                    ...
                    <td className="order-item">{item.name} </td>
                    <td>
